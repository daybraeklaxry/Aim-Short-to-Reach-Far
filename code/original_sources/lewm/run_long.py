...
def episode(task, row, ordinal, source, p, api, ew, context, native, counter):
    ...
    try:
        ...
        arrays = dict(actions=np.asarray(actions,np.float32).reshape(-1,native.action_dim), score_current=np.asarray(encoded_states,np.float32).reshape(-1,192), goal_encoding=np.asarray(cached['goal'],np.float32), raw_action_bounds=np.stack([low,high]))
        return trace, arrays
    finally:
...
def main(a):
    ...
    completed=0
    try:
        with (output/'outcomes.jsonl').open('x') as records:
            assignments=[(o,c) for o in range(128) for c in p['conditions']] if a.mode=='evaluate' else [(0,'clean'),(0,'prefix_a')]
    ...
    runtime.write(output/'summary.json',dict(completed=True,outcomes=completed,elapsed_seconds=time.perf_counter()-started))

if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--task',choices=('cube','pusht','reacher','tworoom'),required=True)
    p.add_argument('--output',type=Path,required=True);p.add_argument('--mode',choices=('smoke','evaluate'),required=True)
    main(p.parse_args())
