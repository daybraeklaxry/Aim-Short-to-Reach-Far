"""Observed-endpoint control with the identical legal recorded-prior CEM."""
import numpy as np
import torch
...
def cem(native, rgb, current, target, generator, normalized_prior, normalized_bounds, raw_bounds):
    ...
    assert work_calls == ITERATIONS + 1 and work_transitions == SAMPLES * ITERATIONS + 2
...
class ValidObservedCEM:
    ...
    def choose(self, rgb, sources, raw, normalized, generator):
        ...
        detail.update(prior_source=int(sources[0]),
                      recorded_prior_projected=prior_projected,
                      recorded_prior_projection_max_abs=float(np.max(np.abs(effective_raw_prior-raw[0]))),
                      target_index=int(sources[0])+BLOCK,
                      target_definition='native encoding of the original recorded source+5 observation',
                      source_prediction_misses=0,
        ...
        arrays = dict(score_current=current[0].cpu().numpy(), score_target=target[0].cpu().numpy(),
                      proposed_raw_actions=chosen.copy(), normalized_prior=effective_normalized_prior.copy(),
                      effective_raw_prior=effective_raw_prior.copy(),
                      normalized_selected=np.asarray(detail['normalized_selected'],np.float32),
                      observed_target_embedding=target[0].cpu().numpy(),
