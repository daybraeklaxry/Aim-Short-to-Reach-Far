...
def main(a):
    ...
    runtime.write(output/'protocol.json',dict(task=a.task,base_study=str(BASE),memory_boundary=boundary,conditions=p['conditions'],selectors=SELECTORS,action_mapping='Same raw-action Box projection as main study, before prediction and physical execution.',diagnostic_population='All eight candidates complete five primitives; additionally report the no-candidate-clipping subset.',continuation='Selector changes only the first block, then identical live-state Direct retrieval to the original total budget.',replication='Exact main first-decision predictions/ranks and full Direct actions are checked for every policy-entered query.',mode=a.mode))
