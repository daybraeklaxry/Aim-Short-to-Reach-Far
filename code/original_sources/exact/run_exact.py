...
from exact_effect import summarize_effects, SELECTORS
...
def evaluate(task,row,ordinal,source,p,api,ew,context,native,scorer):
...
def main(a):
    ...
    output=a.output/a.task;output.mkdir(parents=True,exist_ok=True)
    assignments=[(i,c) for i in range(128) for c in p['conditions']] if a.mode=='evaluate' else [(0,'clean'),(0,'prefix_a')]
    runtime.write(output/'protocol.json',dict(task=a.task,base_study=str(BASE),memory_boundary=boundary,conditions=p['conditions'],selectors=SELECTORS,action_mapping='Same clipping to the action bounds as main study, before prediction and physical execution.',diagnostic_population='All eight candidates complete five primitives; additionally report the no-candidate-clipping subset.',continuation='Selector changes only the first block, then identical live-state Direct retrieval to the original total budget.',replication='Exact main first-decision predictions/ranks and full Direct actions are checked for every policy-entered query.',mode=a.mode))
    with ew.h5py.File(ew.v6._data_path(context),'r') as handle,(output/'outcomes.jsonl').open('x') as records:
        ...
        for count,(ordinal,condition) in enumerate(assignments,1):
            ...
            result,arrays=evaluate(a.task,row,ordinal,sources[(row['record_id'],condition)],p,api,ew,context,native,scorer)
            ...
            print(json.dumps(dict(task=a.task,mode=a.mode,completed=count,expected=len(assignments),elapsed_seconds=round(time.perf_counter()-start,1))),flush=True)
    runtime.write(output/'summary.json',dict(completed=True,outcomes=len(assignments),main_prediction_replay_exact=True,direct_continuation_replay_exact=True,elapsed_seconds=time.perf_counter()-start))

if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--task',choices=('cube','pusht','reacher','tworoom'),required=True)
    p.add_argument('--mode',choices=('smoke','evaluate'),required=True);p.add_argument('--output',type=Path,required=True)
    main(p.parse_args())
