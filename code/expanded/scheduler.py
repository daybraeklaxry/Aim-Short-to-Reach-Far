...
def cmd(job):
 ...
 if kind=='probe':
  return ['run_suite.py','--task',task,'--job-id',c['target_job'],'--probe'],None,ROOT/'jobs'/c['target_job']/'probe.json'
 if kind=='goal_encoding':
  return ['encode_missing_goals.py','--task',task],None,ROOT/'goal_encodings'/task/'summary.json'
 if kind=='quality':
  if not (ROOT/'training'/task/'selected.json').exists():return None,'awaiting validation-only target selection',None
  return ['target_quality.py','--task',task],None,ROOT/'target_quality'/task/'summary.json'
 if kind=='train':
 ...
 if kind=='exact' and (CODE/'run_exact_v2.py').exists() and (CODE/'exact_ready.json').exists():
  if not (ROOT/'training'/task/'selected.json').exists():return None,'awaiting validation-only target selection',None
...
manifest=read(ROOT/'queue-manifest.json');jobs=manifest['jobs']+read(ROOT/'preflight-jobs.json')+read(ROOT/'quality-jobs.json')
...
while True:
 try:
  select_targets()
  probes=[j for j in jobs if j['kind']=='probe']
  if all((ROOT/'jobs'/j['config']['target_job']/'probe.json').exists() for j in probes):
   receipts=[read(ROOT/'jobs'/j['config']['target_job']/'probe.json') for j in probes]
   if all(d.get('complete') and d['episodes']==3 for d in receipts):
    write(CODE/'suite_ready.json',dict(preflight_jobs=[j['config']['target_job'] for j in probes],peak_reserved_bytes=max(d['peak_reserved_bytes'] for d in receipts),validated=True))
  for job in jobs:
   s=state['jobs'][job['id']]
   ...
   if summary and summary.exists() and (read(summary).get('complete') or read(summary).get('completed')):
    ...
    s.setdefault('attempt_history',[]).append(dict(attempt=s['attempts'],log=s['log'],oom=oom,terminal_utc=s['terminal_utc']))
  ...
  for job in sorted(jobs,key=lambda j:(j['priority'],0 if j['kind']=='train' else 1,j['config'].get('start',0),j['id'])):
   ...
   demand=12000 if job['kind']=='train' else 9000 if job['kind']=='long' else 71680 if job['kind']=='exact' else 30000
   if job['kind'] not in ['train','long','exact','probe'] and (CODE/'suite_ready.json').exists():
    measured=read(CODE/'suite_ready.json')['peak_reserved_bytes']/2**20
    demand=max(12000,int(measured+1024))
   if job['kind']=='exact' and (CODE/'exact_allocator_ready.json').exists():
    demand=int(read(CODE/'exact_allocator_ready.json')['admission_mib'])
   available=[g for g,(free,util) in resources.items() if demand<.85*free and counts[g]<min(2,state.get('gpu_worker_caps',{}).get(str(g),2))]
