...
def rollout(args, protocol, api, ew, context, native, construction, scorer, counter, row, source, arm, ordinal, original_cem):
    ...
    try:
        while not success and not stopped and len(actions) < study['budget']:
            ...
            remaining = int(row['horizon']) if getattr(scorer, 'fixed_span', False) else max(5, int(row['horizon']) - step)
            scorer.remaining = remaining
            sources, _, retrieval_cost = context.get('policy_bank', context['bank']).nearest_topk(current, goal, remaining, 8)
            sources = np.asarray(sources[0], np.int64)
            bank = context.get('policy_bank', context['bank'])
            ...
            if step == 0 and not getattr(scorer, 'changed_retrieval', False):
